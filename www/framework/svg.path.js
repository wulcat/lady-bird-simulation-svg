...
(function() {
	...
	SVG.extend(SVG.Path, {
		 ...
		 *	a function
		 */
		drawAnimatedOriginal: function(options){
			options = options || {};
		 ...
		 *	a function
		 */
		drawAnimatedFade: function(options){
			options = options || {};
			options.duration = options.duration || '1000';
			options.easing = options.easing || '<>';
			options.delay = options.delay || 0;
			...
			var fx = this.animate(options.duration, options.easing, options.delay).after(function(){
				this.animate(options.duration , ">").stroke({width:"0"}) ;
			});
